...
import { Camera, CheckCircle2, LockKeyhole, MonitorCog, Server } from "lucide-react";
import { AVAILABLE_STATIONS, APP_NAME, APP_SUBTITLE, DEMO_PIN, DEMO_SERVER_URL, ROUTES } from "@/lib/constants";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => setClock(format(new Date(), "HH:mm:ss")), 1000);
  ...
  function submit(value = pin.join("")) {
    ...
    setLoading(true);
    setConfig({ stationId: station.id, stationLabel: station.label, serverUrl: DEMO_SERVER_URL });
    ...
    <main className="auth-page">
      <header className="auth-header">
        ...
        <div className="topbar-right">
          ...
          <span className="status-pill"><Server size={13} /> {DEMO_SERVER_URL}</span>
          <span className="status-pill"><Camera size={13} /> Camera bus ready</span>
          <span className="status-pill">{clock}</span>
          <ThemeToggle compact />
        </div>
      </header>
          ...
          <div className="panel">
            ...
            <div className="panel-body grid" style={{ gap: 20 }}>
              <label className="field">
                ...
                <select className="select" value={station.id} onChange={event => {
                  const next = AVAILABLE_STATIONS.find(item => item.id === event.target.value);
                  ...
                  {pin.map((digit, index) => (
                    <input
                      ...
                      onKeyDown={event => {
                        ...
                        if (event.key === "Enter" && pin.every(Boolean)) submit();
                      }}
                    />
                  ))}
                </div>
              ...
              <div style={{ minHeight: 24, color: error ? "var(--red)" : "var(--green)", fontFamily: "var(--font-ibm-plex-mono)", fontSize: 12 }}>
                {error || (loading ? "Access granted. Loading setup..." : "Local-only prototype. No cloud authentication is used.")}
              </div>

              <button className="btn btn-primary" disabled={loading || !pin.every(Boolean)} onClick={() => submit()}>
                {loading ? <CheckCircle2 size={17} /> : <LockKeyhole size={17} />}
