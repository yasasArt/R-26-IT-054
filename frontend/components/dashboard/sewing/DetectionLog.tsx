...
export function DetectionLog() {
  ...
  const cycleColor = (s: number) =>
    s <= CYCLE_TIME.TARGET_SECONDS  ? "#22C55E"
    : s <= CYCLE_TIME.WARNING_SECONDS ? "#FACC15"
    : "#EF4444";

  const statusColor = (s: string) =>
    s === "active" ? "#22C55E" : s === "rework" ? "#FACC15" : "#EF4444";
    ...
    >
      ...
      >
        ...
        <span className="font-mono text-dim" style={{ fontSize: 9 }}>
          {detectionLog.length} records
        </span>
      </div>

      {/* Column headers */}
      <div
        className="grid font-mono text-dim uppercase px-5 py-2"
        style={{
          fontSize: 8, letterSpacing: "0.1em",
          gridTemplateColumns: "48px 90px 70px 84px 56px 1fr",
          gap: "0 12px",
      ...
      <div className="overflow-y-auto" style={{ maxHeight: 248 }}>
        ...
        {rows.map((ev, i) => (
          <div
            key={ev.id}
            className="grid items-center px-5 py-2.5 font-mono"
            style={{
              gridTemplateColumns: "48px 90px 70px 84px 56px 1fr",
              gap: "0 12px",
              fontSize: 11,
              borderBottom: i < rows.length - 1 ? "1px solid rgba(36,48,68,0.3)" : "none",
              background: i === 0 && page === 0 ? "rgba(34,197,94,0.03)" : "transparent",
            }}
          >
            ...
            <span className="text-text-muted">
              {formatTime(new Date(ev.timestamp))}
            </span>
            <span className="font-semibold" style={{ color: cycleColor(ev.cycleTimeSeconds) }}>
              {ev.cycleTimeSeconds}s
            </span>
            <span
              style={{
                fontSize: 9, letterSpacing: "0.06em", textTransform: "uppercase",
                color: statusColor(ev.operatorStatus),
              }}
            >
              {ev.operatorStatus}
            </span>
            ...
            <div className="flex items-center gap-1">
              {[
                { lbl: "A", active: ev.signalA, color: "#22C55E" },
                { lbl: "B", active: ev.signalB, color: "#3B82F6" },
              ].map(sig => (
                <span
                  key={sig.lbl}
                  className="font-mono"
                  style={{
                    ...
                    background: sig.active ? `${sig.color}18` : "rgba(255,255,255,0.03)",
                    border: `1px solid ${sig.active ? `${sig.color}50` : "#243044"}`,
                    color: sig.active ? sig.color : "#3A4A5C",
                  }}
                >
                  {sig.lbl}
                </span>
