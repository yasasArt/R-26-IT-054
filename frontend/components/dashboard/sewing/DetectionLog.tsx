...
export function DetectionLog() {
  ...
  return (
    <div className="overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: 12 }}>
      {/* Header */}
      ...
      {/* Column headers */}
      <div className="grid font-mono text-dim uppercase px-5" style={{ fontSize: 8, letterSpacing: "0.1em", gridTemplateColumns: "48px 90px 70px 84px 56px 1fr", gap: "0 12px", borderBottom: "1px solid var(--card-border)", padding: "10px 20px" }}>
          ...
          <div key={ev.id} className="grid items-center font-mono" style={{ gridTemplateColumns: "48px 90px 70px 84px 56px 1fr", gap: "0 12px", fontSize: 11, borderBottom: i < rows.length - 1 ? "1px solid rgba(36,48,68,0.3)" : "none", background: i === 0 && page === 0 ? "rgba(34,197,94,0.03)" : "transparent", padding: "11px 20px" }}>
            ...
            <div className="flex items-center gap-1">
              {[{ lbl: "A", active: ev.signalA, color: "#22C55E" }, { lbl: "B", active: ev.signalB, color: "#3B82F6" }].map(sig => (
                <span key={sig.lbl} className="font-mono" style={{ fontSize: 8, padding: "2px 6px", borderRadius: 3, background: sig.active ? `${sig.color}18` : "rgba(255,255,255,0.03)", border: `1px solid ${sig.active ? `${sig.color}50` : "var(--card-border)"}`, color: sig.active ? sig.color : "var(--dim)" }}>{sig.lbl}</span>
              ))}
            </div>
          </div>
        ))}
