...
const CFG: Record<OperatorStatus, { label: string; icon: string; color: string; bg: string; border: string; desc: string }> = {
  ...
  idle:     { label: "IDLE",     icon: "⏸", color: "var(--muted)", bg: "rgba(107,122,141,0.08)", border: "rgba(107,122,141,0.22)", desc: "Waiting for material"   },
...
export function OperatorStatusCard() {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 shrink-0" style={{ height: 48, background: "var(--surface)", borderBottom: `1px solid ${cfg.border}` }}>
