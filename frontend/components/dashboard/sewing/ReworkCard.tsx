...
export function ReworkCard() {
  ...
  return (
    <div
      ...
      style={{ background: "#1A2536", border: `1px solid ${reworkActive ? "rgba(250,204,21,0.4)" : "#243044"}`, borderRadius: 12, boxShadow: reworkActive ? "0 0 18px rgba(250,204,21,0.1)" : "none", transition: "border-color 0.3s, box-shadow 0.3s" }}
    >
      {/* Header */}
      <div className="flex shrink-0 items-center justify-between px-4" style={{ height: 48, background: "#131B26", borderBottom: `1px solid ${reworkActive ? "rgba(250,204,21,0.2)" : "#243044"}` }}>
        <span className="font-mono text-text-muted uppercase" style={{ fontSize: 9, letterSpacing: "0.14em" }}>Rework</span>
        <div className={reworkActive ? "animate-pulse-slow rounded-full" : "rounded-full"} style={{ width: 7, height: 7, background: reworkActive ? "#FACC15" : "#3A4A5C", boxShadow: reworkActive ? "0 0 6px #FACC15" : "none" }} />
      </div>

      {/* Body */}
      <div style={{ padding: "16px 18px", flex: 1, display: "flex", flexDirection: "column", gap: 12 }}>
        <div className="flex items-center justify-between rounded-lg transition-all duration-300" style={{ padding: "11px 14px", background: reworkActive ? "rgba(250,204,21,0.08)" : "rgba(255,255,255,0.02)", border: `1px solid ${reworkActive ? "rgba(250,204,21,0.25)" : "#243044"}` }}>
          <div className="font-mono font-bold" style={{ fontSize: 13, color: reworkActive ? "#FACC15" : "#3A4A5C" }}>{reworkActive ? "ACTIVE" : "Normal"}</div>
