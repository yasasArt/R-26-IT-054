...
type TickEvent = { id: string; time: Date; label: string; kind: "det" | "rework" | "down" };
...
function buildEvents(dets: IDetectionEvent[], iot: IIoTEvent[]): TickEvent[] {
  return [
    ...
    ...iot.slice(0, 8).map(e => ({
      ...
      label: e.type === "rework_triggered" ? "Rework triggered by operator"
  ...
  ].sort((a, b) => b.time.getTime() - a.time.getTime()).slice(0, 14);
}
  ...
  return (
    <div className="flex flex-col overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: 12 }}>
      ...
      <div ref={listRef} className="overflow-y-auto" style={{ flex: 1 }}>
        {events.length === 0 && (
          <div className="flex items-center justify-center font-mono text-dim" style={{ height: 64, fontSize: 11 }}>Waiting for events…</div>
        )}
        {events.map((ev, i) => (
          <div key={ev.id} className="flex items-start gap-3 px-5 py-3" style={{ borderBottom: i < events.length - 1 ? "1px solid rgba(36,48,68,0.35)" : "none", background: i === 0 ? "rgba(255,255,255,0.015)" : "transparent" }}>
            <div className="rounded-full shrink-0" style={{ width: 5, height: 5, marginTop: 5, background: KIND_COLOR[ev.kind], boxShadow: `0 0 5px ${KIND_COLOR[ev.kind]}80` }} />
            <div className="flex-1 min-w-0">
              <div className="font-display text-text-primary truncate" style={{ fontSize: 12 }}>{ev.label}</div>
            </div>
            <span className="font-mono text-dim shrink-0" style={{ fontSize: 9.5 }}>{formatTime(ev.time)}</span>
          </div>
        ))}
      </div>
    </div>
