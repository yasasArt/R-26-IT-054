...
function groupByHour(records: { timestamp: Date | string }[]) {
  ...
  records.forEach(r => { const h = format(new Date(r.timestamp), "HH:00"); groups[h] = (groups[h] || 0) + 1; });
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 4, right: 14, bottom: 0, left: -14 }}>
              ...
              <Bar dataKey="count" radius={[3, 3, 0, 0]} maxBarSize={42} isAnimationActive={false}>
                {data.map((entry, i) => (
                  <Cell key={i} fill={entry.current ? "#3B82F6" : "rgba(59,130,246,0.28)"} stroke={entry.current ? "rgba(59,130,246,0.6)" : "none"} strokeWidth={1} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
