...
def test_production_remains_blocked_without_real_models_and_iot(
    ...
    response = configured_client.post(
        "/api/sessions",
        json={"employee_id": employee["id"], "target_pieces": 50, "operation_type": "SLEEVE", "estimated_cycle_time_sec": 24, "session_mode": "PRODUCTION"},
    )
...
def test_validation_requires_an_explicitly_approved_simulated_controller(
    ...
    response = configured_client.post(
        "/api/sessions",
        json={"employee_id": employee["id"], "target_pieces": 50, "operation_type": "POCKET", "estimated_cycle_time_sec": 30, "session_mode": "VALIDATION"},
    )
    assert response.status_code == 409
...
def test_only_one_session_can_be_active(
    ...
    response = configured_client.post(
        "/api/sessions",
        json={"employee_id": employee["id"], "target_pieces": 20, "operation_type": "COLLAR", "estimated_cycle_time_sec": 20, "session_mode": "VALIDATION"},
    )
    assert validation_session["status"] == "ACTIVE"
    assert response.status_code == 409


def test_session_requires_a_supported_operation_and_positive_cycle_estimate(
    configured_client: TestClient, employee: dict
) -> None:
    invalid_operation = configured_client.post(
        "/api/sessions",
        json={
            "employee_id": employee["id"],
            "target_pieces": 20,
            "operation_type": "BUTTON",
    ...
    invalid_estimate = configured_client.post(
        "/api/sessions",
        json={
            "employee_id": employee["id"],
            "target_pieces": 20,
...
def test_cycle_status_is_red_when_actual_cycle_exceeds_estimate(
    configured_client: TestClient, validation_session: dict
) -> None:
    started = parse_utc(validation_session["started_at"]) + timedelta(seconds=2)
    response = configured_client.post(
        f"/api/sessions/{validation_session['id']}/pieces",
        json={
            "sewing_started_at": format_utc(started),
            "completed_at": format_utc(started + timedelta(seconds=25)),
            "event_source": "VALIDATION",
        },
    )

    assert response.status_code == 201
    assert response.json()["cycle_status"] == "OVER_ESTIMATE"
    assert response.json()["cycle_difference_sec"] == 5.0
...
def test_first_piece_has_a_real_cycle_duration_and_appears_in_dashboard(
    ...
    assert dashboard["piece_events"][0]["cycle_status"] == "WITHIN_ESTIMATE"
    assert dashboard["piece_events"][0]["operation_type"] == "COLLAR"
    assert dashboard["session"]["estimated_cycle_time_sec"] == 20
...
def test_each_actual_sewing_start_excludes_idle_gaps_from_average_cycle_time(
    ...
    assert dashboard["session"]["average_cycle_status"] == "WITHIN_ESTIMATE"
