...
def test_combined_analytics_aggregates_sessions_cycles_and_iot_events(
    ...
    assert payload["summary"]["within_estimate_count"] == 2
    assert payload["summary"]["over_estimate_count"] == 0
    assert payload["summary"]["cycle_compliance_percent"] == 100.0
...
def test_analytics_supports_employee_session_line_and_mode_filters(
    ...
    matching = configured_client.get(
        ...
        "&sewing_line=Line%20A&session_mode=VALIDATION&operation_type=COLLAR"
    ).json()
    ...
    other_operation = configured_client.get("/api/analytics?operation_type=POCKET").json()
    assert other_operation["summary"]["session_count"] == 0
...
def test_excel_export_contains_professional_industry_report_tabs(
    ...
    assert workbook["Session Register"]["W5"].value == "COLLAR"
    assert workbook["Session Register"]["X5"].value == 20
    ...
    assert workbook["Garment Cycle Detail"]["J5"].value == "COLLAR"
    assert workbook["Garment Cycle Detail"]["M5"].value == "WITHIN_ESTIMATE"
