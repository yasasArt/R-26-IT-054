...
def serialize_session(row: sqlite3.Row) -> dict[str, Any]:
    session = enrich_session_cycle_status(row)
...
def create_application(settings: Settings | None = None) -> FastAPI:
        ...
        try:
            with transaction(connection):
                cursor = connection.execute(
                    ...
                    "sewing_line, workstation_id, camera_id, camera_label, target_pieces, operation_type, "
                    "estimated_cycle_time_sec, session_mode, simulated_iot, started_at, created_at) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        ...
                        configuration["camera_label"], payload.target_pieces, payload.operation_type,
                        payload.estimated_cycle_time_sec, payload.session_mode,
    ...
    def list_pieces(session_id: int, connection: Database) -> list[dict[str, Any]]:
        ...
        return [
            enrich_piece_event(
                ...
                estimated_cycle_time_sec=session["estimated_cycle_time_sec"],
            )
            for row in connection.execute(
    ...
    def analytics(
        ...
        operation_type: Annotated[str | None, Query(pattern=r"^(COLLAR|POCKET|SLEEVE)$")] = None,
    ) -> dict[str, Any]:
        return build_analytics(
            ...
            session_mode=session_mode, operation_type=operation_type,
        )
