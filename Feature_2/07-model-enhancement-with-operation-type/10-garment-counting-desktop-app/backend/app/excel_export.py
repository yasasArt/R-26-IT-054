...
def report_scope(payload: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "operation": filters.get("operation_type") or "All operations",
...
def add_overview(workbook: Workbook, payload: dict[str, Any]) -> None:
    ...
    metadata = [
        ...
        ("Operation", scope["operation"], "From date", scope["start"]),
        ("Through date", scope["end"], "", ""),
    ]
    ...
    overview_metrics = [
        ...
        ("Cycles within estimate", summary["within_estimate_count"], "Cycle compliance", (summary["cycle_compliance_percent"] or 0) / 100),
    ]
    ...
    worksheet["H18"].number_format = PERCENT_FORMAT
    worksheet.merge_cells("A20:H20")
    note = worksheet["A20"]
    note.value = (
        "Blue/within-estimate cycles are at or below the selected session benchmark; "
        "red/over-estimate cycles are above it. All timestamps are shown in UTC."
    )
    note.font = Font(name="Aptos", size=9, color=MUTED, italic=True)
    note.alignment = Alignment(wrap_text=True, vertical="center")
    worksheet.row_dimensions[20].height = 34
...
def build_workbook(payload: dict[str, Any]) -> bytes:
    ...
    session_headers = [
        ...
        "Operation", "Estimated Cycle (sec)", "Average vs Estimate (sec)", "Cycle Status",
    ]
    session_rows = []
    for row_number, session in enumerate(payload["sessions"], start=5):
        session_rows.append([
            ...
            session.get("operation_type") or "Legacy / not recorded",
            session.get("estimated_cycle_time_sec"),
            session.get("average_cycle_difference_sec"),
            session.get("average_cycle_status"),
        ])
    ...
    piece_headers = [
        ...
        "Operation", "Estimated Cycle (sec)", "Difference (sec)", "Cycle Status",
    ]
    piece_rows = []
    ...
    for row_number, event in enumerate(payload["piece_events"], start=5):
        ...
        piece_rows.append([
            ...
            event.get("operation_type") or "Legacy / not recorded",
            event.get("estimated_cycle_time_sec"), event.get("cycle_difference_sec"),
            event.get("cycle_status"),
        ])
