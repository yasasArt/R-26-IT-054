...
MIGRATIONS: tuple[tuple[int, str], ...] = (
    ...
    (
        2,
        """
        -- Version 2 existed in some pre-enhancement databases. The actual
        -- operation-context upgrade is therefore applied idempotently by
        -- ensure_operation_context_schema() after numbered migrations run.
        SELECT 1;
        """,
    ),
)
...
def ensure_operation_context_schema(connection: sqlite3.Connection) -> None:
    """Repair legacy databases whose earlier schema already used version 2."""

    columns = {
        str(row["name"])
        for row in connection.execute("PRAGMA table_info(production_sessions)").fetchall()
    }
    ...
    with transaction(connection):
            ...
            connection.execute(
                ...
                ALTER TABLE production_sessions
                    ...
                    CHECK (
                        ...
                        OR operation_type IN ('COLLAR', 'POCKET', 'SLEEVE')
                    )
                """
            )

        if "estimated_cycle_time_sec" not in columns:
            connection.execute(
                """
                ALTER TABLE production_sessions
                    ADD COLUMN estimated_cycle_time_sec REAL
                    CHECK (
                        estimated_cycle_time_sec IS NULL
                        OR estimated_cycle_time_sec > 0
                    )
                """
            )
        ...
        connection.execute(
            """
            CREATE INDEX IF NOT EXISTS sessions_operation_start
                ON production_sessions(operation_type, started_at)
            """
        )
        connection.execute(
...
def migrate(connection: sqlite3.Connection) -> None:
    ...
    ensure_operation_context_schema(connection)
