...
import { CycleStatusBadge, EmptyState, InlineNotice, MetricCard, ModeBadge, operationLabel, PageHeading } from "../components/OperatorUi";
    ...
    <div className="screen-stack">
      ...
      <section className="panel analytics-filter-panel">
        <div className="filter-heading analytics-filter-heading">
          <div><h2>Filter production results</h2><p>Combine employee, operation, session, line, date, and session-type filters.</p></div>
        ...
        <div className="analytics-filter-grid">
          ...
          <label><span>Operation</span><select className="form-select" value={filters.operation_type} onChange={(event) => setFilters((current) => ({ ...current, operation_type: event.target.value as OperationType | "" }))}><option value="">All operations</option><option value="COLLAR">Collar</option><option value="POCKET">Pocket</option><option value="SLEEVE">Sleeve</option></select></label>
      ...
      <div className="metric-grid metric-grid-six">
        ...
        <MetricCard icon="clock" label="Within estimate" value={summary?.cycle_compliance_percent === null || summary?.cycle_compliance_percent === undefined ? "—" : `${formatNumber(summary.cycle_compliance_percent, 1)}%`} detail={`${summary?.within_estimate_count ?? 0} compliant cycles`} tone="blue" />
      </div>
          ...
          <div className="table-wrap">
            {activeTab === "sessions" ? (
              <table className="operator-table analytics-table">
                <thead><tr><th>Session</th><th>Employee</th><th>Operation</th><th>Sewing line</th><th>Type</th><th>Pieces</th><th>Achievement</th><th>Avg / estimate</th><th>Cycle status</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>{payload.sessions.map((session) => (
                  <tr key={session.id} className="analytics-clickable-row" tabIndex={0} onClick={() => void openSessionDetails(session)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") void openSessionDetails(session); }}>
                    <td><strong>{session.session_code}</strong><span className="table-subline">{formatDateTime(session.started_at)}</span></td><td>{session.employee_name}</td><td>{operationLabel(session.operation_type)}</td><td>{session.sewing_line}</td><td><ModeBadge mode={session.session_mode} /></td><td>{session.total_pieces} / {session.target_pieces}</td><td>{formatNumber(session.achievement_percent, 1)}%</td><td>{session.average_cycle_seconds === null ? "—" : `${formatNumber(session.average_cycle_seconds, 1)} / ${session.estimated_cycle_time_sec === null ? "—" : formatNumber(session.estimated_cycle_time_sec, 1)} sec`}</td><td><CycleStatusBadge status={session.average_cycle_status} /></td><td><ModeBadge mode={session.status} /></td>
            ...
            {activeTab === "pieces" ? <table className="operator-table analytics-table"><thead><tr><th>Session ID</th><th>Operation</th><th>Piece</th><th>Cycle / estimate</th><th>Performance</th><th>Completed</th><th>Transition</th><th>Source</th></tr></thead><tbody>{payload.piece_events.map((event) => <tr key={event.id}><td>#{event.session_id}</td><td>{operationLabel(event.operation_type)}</td><td><strong>Piece {event.piece_number}</strong></td><td>{formatNumber(event.cycle_seconds, 2)} / {event.estimated_cycle_time_sec === null ? "—" : formatNumber(event.estimated_cycle_time_sec, 2)} sec</td><td><CycleStatusBadge status={event.cycle_status} /></td><td>{formatDateTime(event.completed_at)}</td><td>SEWING → IDLE_SETUP</td><td><ModeBadge mode={event.event_source} /></td></tr>)}</tbody></table> : null}
