...
export function LiveDashboardScreen({ session, onSessionUpdated, onSessionCompleted, onCreateSession }: Props) {
  ...
  const latestPiece = dashboard?.piece_events.at(-1) ?? null;
  const averageCycleTone = current.average_cycle_status === "OVER_ESTIMATE" ? "rose" : "blue";
  ...
  return (
    <div className="screen-stack">
      <PageHeading
        ...
        description={`${current.employee_name} · ${current.sewing_line} · ${operationLabel(current.operation_type)} · ${current.workstation_id}`}
        action={
      ...
      <section className="production-hero">
        <div className="production-hero-copy">
          ...
          <p>
            {current.session_code} · {operationLabel(current.operation_type)} · Estimated cycle {current.estimated_cycle_time_sec === null ? "not recorded" : `${formatNumber(current.estimated_cycle_time_sec, 1)} sec`}
          </p>
      ...
      <div className="metric-grid metric-grid-five">
        ...
        <MetricCard
          icon="clock"
          label="Avg cycle"
          value={current.average_cycle_seconds === null ? "—" : `${formatNumber(current.average_cycle_seconds, 1)}s`}
          detail={current.estimated_cycle_time_sec === null ? "No estimate recorded" : `Estimate ${formatNumber(current.estimated_cycle_time_sec, 1)}s`}
          tone={averageCycleTone}
        />
      ...
      <section className={`cycle-benchmark-panel ${latestPiece?.cycle_status === "OVER_ESTIMATE" ? "is-over" : "is-within"}`}>
        <div>
          <span className="eyebrow">LIVE CYCLE BENCHMARK</span>
          <h2>{latestPiece ? `Piece #${latestPiece.piece_number}` : "Waiting for the first completed piece"}</h2>
          <p>{operationLabel(current.operation_type)} operation · Blue is within estimate, red is over estimate.</p>
        </div>
        <div className="cycle-benchmark-values">
          <span><small>Estimated</small><strong>{current.estimated_cycle_time_sec === null ? "—" : `${formatNumber(current.estimated_cycle_time_sec, 1)}s`}</strong></span>
          <span><small>Latest actual</small><strong>{latestPiece ? `${formatNumber(latestPiece.cycle_seconds, 1)}s` : "—"}</strong></span>
          {latestPiece ? <CycleStatusBadge status={latestPiece.cycle_status} /> : null}
        </div>
      </section>
      ...
      <div className="chart-split-grid">
        <section className="panel chart-panel">
          ...
          <CycleTimeChart events={dashboard?.piece_events ?? []} estimatedCycleTime={current.estimated_cycle_time_sec} />
        </section>
        <section className="panel chart-panel">
      ...
      <div className="activity-split-grid">
        <section className="panel event-panel">
          ...
          {(dashboard?.piece_events.length ?? 0) ? (
            <div className="table-wrap"><table className="operator-table"><thead><tr><th>Piece</th><th>Cycle time</th><th>Performance</th><th>Recorded</th></tr></thead><tbody>
              {[...(dashboard?.piece_events ?? [])].reverse().slice(0, 8).map((event) => (
                <tr key={event.id}><td>#{event.piece_number}</td><td>{formatNumber(event.cycle_seconds, 1)} sec</td><td><CycleStatusBadge status={event.cycle_status} /></td><td>{formatTime(event.completed_at)}</td></tr>
              ))}
            </tbody></table></div>
