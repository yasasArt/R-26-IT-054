...
import { CycleStatusBadge, EmptyState, InlineNotice, MetricCard, ModeBadge, operationLabel } from "./OperatorUi";
    ...
    <div className="analytics-detail-backdrop" role="presentation" onMouseDown={(event) => {
      ...
      <section className="analytics-detail-modal" role="dialog" aria-modal="true" aria-labelledby="session-detail-title">
        ...
        <div className="analytics-detail-meta">
          ...
          <span><strong>Operation</strong>{operationLabel(session.operation_type)}</span>
          <span><strong>Estimated cycle</strong>{session.estimated_cycle_time_sec === null ? "Not recorded" : `${formatNumber(session.estimated_cycle_time_sec, 1)} sec`}</span>
        ...
        {loading ? <p className="table-empty-note">Loading garment cycles and controller activity…</p> : detail ? (
          <>
            <div className="metric-grid analytics-detail-metrics">
              ...
              <MetricCard icon="target" label="Average cycle" value={detail.session.average_cycle_seconds === null ? "—" : `${formatNumber(detail.session.average_cycle_seconds, 1)} sec`} detail={`${detail.piece_events.length} measured cycles`} tone={detail.session.average_cycle_status === "OVER_ESTIMATE" ? "rose" : "blue"} />
              ...
              {detail.piece_events.length ? (
                <div className="table-wrap analytics-detail-table-wrap">
                  <table className="operator-table analytics-table">
                    <thead><tr><th>Piece</th><th>Cycle time</th><th>Performance</th><th>Sewing started</th><th>Completed</th><th>Source</th></tr></thead>
                    <tbody>{detail.piece_events.map((event) => (
                      <tr key={event.id}><td><strong>Piece {event.piece_number}</strong></td><td>{formatNumber(event.cycle_seconds, 2)} sec</td><td><CycleStatusBadge status={event.cycle_status} /></td><td>{event.sewing_started_at ? formatDateTime(event.sewing_started_at) : "—"}</td><td>{formatDateTime(event.completed_at)}</td><td><ModeBadge mode={event.event_source} /></td></tr>
                    ))}</tbody>
                  </table>
                </div>
